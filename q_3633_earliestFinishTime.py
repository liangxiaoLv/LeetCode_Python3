...
#
#
#  Related Topics 贪心 数组 双指针 二分查找 排序 👍 17 👎 0
# leetcode submit region begin(Prohibit modification and deletion)
from cmath import inf


def solve(start1: List[int], dur1: List[int], start2: List[int], dur2: List[int]):
    finish1 = inf
    finish2 = inf
    for i in range(len(start1)):
        finish1 = min(finish1, start1[i] + dur1[i])
    for j in range(len(start2)):
        finish2 = min(finish2, max(finish1, start2[j]) + dur2[j])  # 1结束的情况下，2开始了，就是接着来，没有就等到2开始
    return finish2
class Solution:
